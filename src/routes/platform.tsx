        ...
        <section className="border-b border-white/10 bg-white/5">
          <Container className="py-20 md:py-28">
            ...
            <div className="mt-12 grid grid-cols-3 gap-4 md:grid-cols-5">
              {sources.map((s) => (
                <div key={s.name} className="rounded-sm border border-white/10 bg-white/5 card-subtle p-6 text-center card-hover">
                  <s.icon className="card-icon mx-auto h-8 w-8 text-copper" />
                  <div className="mt-3 font-serif text-sm text-white">{s.name}</div>
                </div>
              ))}
            </div>
          </Container>
        ...
        <section className="border-b border-white/10">
          <Container className="py-20 md:py-28">
            ...
            <div className="mt-12 space-y-4">
              {loop.map((l, i) => (
                <div key={l.stage} className="flex items-start gap-6 rounded-sm border border-white/10 bg-white/5 card-subtle p-6 card-hover transition-colors hover:border-copper">
                  <div className="label-mono w-8 text-copper">0{i + 1}</div>
                  <div className="flex-1">
                    <div className="font-serif text-xl text-white">{l.stage}</div>
                    <p className="mt-1 text-sm text-white/60">{l.output}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <Container className="py-20 md:py-28">
            ...
            <div className="mt-12 grid gap-6 md:grid-cols-2">
              {capabilities.map((c, i) => (
                <article key={c.t} className="rounded-sm border border-l-2 border-l-copper/30 border-white/10 bg-white/5 card-subtle p-8 card-hover transition-all hover:border-l-copper">
                   <div className="flex items-center gap-3">
                     <c.icon className="card-icon h-5 w-5 text-copper" />
                    <div className="label-mono text-copper">0{i + 1}</div>
                  </div>
                  <h3 className="mt-4 font-serif text-2xl text-white">{c.t}</h3>
                  <div className="mt-4 space-y-3">
                    <div>
                      <div className="label-mono">What it does</div>
                      <p className="mt-1 text-sm text-white">{c.what}</p>
                    </div>
                    <div>
                      <div className="label-mono">Where it applies</div>
                      <p className="mt-1 text-sm text-white">{c.where}</p>
                    </div>
                    <div>
                      <div className="label-mono">Example outputs</div>
                      <p className="mt-1 text-sm text-white">{c.outputs}</p>
                    </div>
                    <div>
                      <div className="label-mono">Deployment</div>
                      <p className="mt-1 text-sm text-white/60">{c.deploy}</p>
                    </div>
                  </div>
                </article>
              ))}
              ...
              <table className="w-full text-left text-sm">
                <thead>
                  <tr className="border-b border-white/10">
                    <th className="pb-4 font-mono text-xs uppercase tracking-wider text-white/60">Model</th>
                    <th className="pb-4 font-mono text-xs uppercase tracking-wider text-white/60">Best for</th>
                    <th className="pb-4 font-mono text-xs uppercase tracking-wider text-white/60">Ownership</th>
                    <th className="pb-4 font-mono text-xs uppercase tracking-wider text-white/60">Operations</th>
                  </tr>
                </thead>
                <tbody>
                  {deploymentModels.map((m) => (
                    <tr key={m.model} className="border-b border-white/10">
                      <td className="py-4 font-serif text-white">{m.model}</td>
                      <td className="py-4 text-white/60">{m.best}</td>
                      <td className="py-4 text-white/60">{m.ownership}</td>
                      <td className="py-4 text-white/60">{m.operations}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <section className="border-b border-white/10">
          <Container className="py-20 md:py-28">
            ...
            <div className="mt-12 flex flex-wrap gap-3">
              {governance.map((g) => (
                <div key={g} className="rounded-sm border border-white/10 bg-white/5 card-subtle px-4 py-3 font-mono text-sm text-white">
                  {g}
                </div>
              ))}
            </div>
